...
@section('content')
    @component('components.breadcrumb')
        ...
        @endslot
        @slot('title')
            {{ __('admin.control') }} {{ __('admin.clients') }} !
        @endslot
    @endcomponent
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="card-title m-0">{{ __('admin.all_clients') }}</h4>
                    @if (isset($super_admin) || isset($client_create))
                        <a href="{{ route('admin.clients.create') }}" class="btn btn-primary button-icon"><i
                                class="fe fe-plus ml-2 font-weight-bolder"></i>{{ __('admin.add_client') }}</a>
                    @endif
                </div>
                            ...
                            @foreach ($clients as $count => $client)
                                <tr data-id="{{ $count + 1 }}">
                                    ...
                                    <td class="align-middle">
                                        <a href="{{ $client->imageLink }}" target="_blanck">
                                            <img src="{{ $client->imageLink }}" alt="{{ __('admin.image') }}"
                                                style="width: 100px;" />
                                        </a>
                                    </td>
                                    ...
                                    <td class="align-middle">
                                        <div class="d-flex">
                                            @if (isset($super_admin) || isset($client_edit))
                                                <form class="d-inline ml-2" action="{{ route('admin.clients.verify') }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    ...
                                                    <button type="submit"
                                                        ...
                                                        @if ($client->blocked == '1') title="{{ __('admin.activate') }}" @else title="{{ __('admin.deactivate') }}" @endif>
                                                        <i
                                                            class="@if ($client->blocked == '1') fas fa-eye @else fas fa-eye-slash @endif"></i>
                                                    </button>
                                                </form>
                                            ...
                                            @endif
                                            @if (isset($super_admin) || isset($client_delete))
                                                <button type="submit"
                                                ...
                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                    <div class="modal-content modal-content-demo">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title">{{ __('admin.delete_client') }}
                                                            </h5>
                                                        ...
                                                        <div class="modal-body">
                                                            <p>{{ __('admin.delete_client_message') }}</p>
                                                        </div>
