...
@section('content')
    @component('components.breadcrumb')
        ...
        @endslot
        @slot('title')
            {{ __('admin.control') }} {{ __('admin.employees') }}!
        @endslot
    @endcomponent
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="card-title m-0">{{ __('admin.all_employees') }}</h4>
                    @if (isset($super_admin) || isset($admin_create))
                        <a href="{{ route('admin.admins.create') }}" class="btn btn-primary button-icon"><i
                            ...
                            @foreach ($admins as $count => $admin)
                                <tr data-id="{{ $count + 1 }}">
                                    ...
                                    <td class="align-middle">
                                        <div class="d-flex">
                                            @if (isset($super_admin) || isset($admin_edit))
                                                <form class="d-inline ml-2" action="{{ route('admin.admins.verify') }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    ...
                                                    <button type="submit"
                                                        ...
                                                        @if ($admin->blocked == '1') title="{{ __('admin.activate') }}" @else title="{{ __('admin.deactivate') }}" @endif>
                                                        <i
                                                            class="@if ($admin->blocked == '1') fas fa-eye @else fas fa-eye-slash @endif"></i>
                                                    </button>
                                                </form>
                                            ...
                                            @endif
                                            @if (isset($super_admin) || isset($admin_delete))
                                                <button type="submit"
                                                ...
                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                    <div class="modal-content modal-content-demo">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title">{{ __('admin.delete_employee') }}</h5>
                                                            <button aria-label="Close" class="close" data-dismiss="modal"
                                                                type="button"><span
                                                                    aria-hidden="true">&times;</span></button>
                                                        </div>
                                                        <div class="modal-body">
                                                            <p>{{ __('admin.delete_employee_message') }}</p>
                                                        </div>
