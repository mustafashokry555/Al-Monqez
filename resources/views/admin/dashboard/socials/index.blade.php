...
@section('content')
    @component('components.breadcrumb')
        @slot('li_1')
            {{ __('admin.all_socials') }}
        @endslot
        @slot('title')
            {{ __('admin.control') }} {{ __('admin.socials') }} !
        @endslot
    @endcomponent
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="card-title m-0">{{ __('admin.all_socials') }}</h4>
                    @if (isset($super_admin) || isset($social_create))
                        <a href="{{ route('admin.socials.create') }}" class="btn btn-primary button-icon"><i
                        ...
                        <tbody>
                            ...
                            @foreach ($socials as $count => $social)
                                <tr data-id="{{ $count + 1 }}">
                                    ...
                                    <td class="align-middle">
                                        {{ $social->displayed == 0 ? __('admin.hidden') : __('admin.displayed') }}</td>
                                    <td class="align-middle">
                                        <div class="d-flex">
                                            @if (isset($super_admin) || isset($social_edit))
                                                <form class="d-inline ml-2" action="{{ route('admin.socials.display') }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    ...
                                                    <button type="submit"
                                                        ...
                                                        @if ($social->displayed == 1) title="{{ __('admin.hide') }}" @else title="{{ __('admin.show') }}" @endif>
                                                        <i
                                                            class="@if ($social->displayed == 1) fas fa-eye-slash @else fas fa-eye @endif"></i>
                                                    </button>
                                                </form>
                                                <a class="btn btn-outline-secondary bg-warning text-dark btn-sm ml-2"
                                                    title="{{ __('admin.edit') }}"
                                                    href="{{ route('admin.socials.edit', [$social->id]) }}">
                                                    <i class="fas fa-pencil-alt"></i>
                                                </a>
                                            @endif
                                            @if (isset($super_admin) || isset($social_delete))
                                                <button type="submit"
                                                ...
                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                    <div class="modal-content modal-content-demo">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title">{{ __('admin.delete_social') }}</h5>
                                                            <button aria-label="Close" class="close" data-dismiss="modal"
                                                                type="button"><span
                                                                    aria-hidden="true">&times;</span></button>
                                                        </div>
                                                        <div class="modal-body">
                                                            <p>{{ __('admin.delete_social_message') }}</p>
                                                        </div>
                                                        <div class="modal-footer">
                                                            <form class="d-inline"
                                                                action="{{ route('admin.socials.destroy') }}"
                                                                method="POST">
                                                                @csrf
                                                                @method('Delete')
