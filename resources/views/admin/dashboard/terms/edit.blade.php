...
@section('content')
    @component('components.breadcrumb')
        @slot('li_1')
            <a class="btn bg-primary text-white btn-sm ml-2" title="{{ __('admin.back') }}" href="{{ route('admin.terms.index') }}">
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title m-0">{{ __('admin.edit_term') }}</h4>
                </div>
                <div class="card-body">
                    <form class="needs-validation" action="{{ route('admin.terms.update') }}" method="POST">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="col-12">
                                    <div class="mb-3">
                                        ...
                                        <input type="input"
                                            ...
                                            value="{{ old("title_$language") ? old("title_$language") : $term->{"title_$language"} }}"
                                            required>
                                ...
                                <div class="col-lg-12">
                                    <div class="mb-3">
                                        ...
                                        <textarea class="form-control @error('description_{{ $language }}') is-invalid @enderror"
                                            id="description_{{ $language }}" name="description_{{ $language }}">{{ old("description_$language") ? old("description_$language") : $term->{"description_$language"} }}</textarea>
                        ...
                        @endforeach
                        <button class="btn btn-primary" type="submit">{{ __('admin.edit_term') }}</button>
                    </form>
                </div>
            </div>
            <!-- end card -->
