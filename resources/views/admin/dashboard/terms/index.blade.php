...
@section('content')
    @component('components.breadcrumb')
        ...
        @endslot
        @slot('title')
            {{ __('admin.control') }} {{ __('admin.terms') }} !
        @endslot
    @endcomponent
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    @if (isset($super_admin) || isset($term_create))
                        <a href="{{ route('admin.terms.create') }}" class="btn btn-primary button-icon"><i
                                class="fe fe-plus ml-2 font-weight-bolder"></i>{{ __('admin.add_term') }}</a>
                    @endif
                </div>
                        ...
                        <tbody>
                            ...
                            @foreach ($terms as $count => $term)
                                <tr data-id="{{ $count + 1 }}">
                                    ...
                                    <td class="align-middle">{{ $term->title }}</td>
                                    <td class="align-middle">
                                        {{ $term->displayed == 0 ? __('admin.hidden') : __('admin.displayed') }}</td>
                                    <td class="align-middle">
                                        <div class="d-flex">
                                            @if (isset($super_admin) || isset($term_edit))
                                                <form class="d-inline ml-2" action="{{ route('admin.terms.display') }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    ...
                                                    <button type="submit"
                                                        ...
                                                        @if ($term->displayed == 1) title="{{ __('admin.hide') }}" @else title="{{ __('admin.show') }}" @endif>
                                                        <i
                                                            class="@if ($term->displayed == 1) fas fa-eye-slash @else fas fa-eye @endif"></i>
                                                    </button>
                                                </form>
                                            ...
                                            @endif
                                            @if (isset($super_admin) || isset($term_delete))
                                                <button type="submit"
                                                    ...
                                                    data-toggle="modal" href="#myModal{{ $term->id }}">
                                        ...
                                        @if (isset($super_admin) || isset($term_delete))
                                            <div class="modal" id="myModal{{ $term->id }}">
                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                    <div class="modal-content modal-content-demo">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title">{{ __('admin.delete_term') }}</h5>
                                                            <button aria-label="Close" class="close" data-dismiss="modal"
                                                                type="button"><span
                                                                    aria-hidden="true">&times;</span></button>
                                                        </div>
                                                        <div class="modal-body">
                                                            <p>{{ __('admin.delete_term_message') }}</p>
                                                        </div>
                                                        <div class="modal-footer">
                                                            <form class="d-inline"
                                                                action="{{ route('admin.terms.destroy') }}" method="POST">
                                                                @csrf
                                                                @method('Delete')
